...
def drawBendingMoment(canvas, member):
    '''
    Draws the bending moment diagram for a given member using polyline.
    '''
    # ข้อมูลพื้นฐาน
    elem = canvas.membersList[member]
    p1 = fn.canvasCoords(canvas, elem.p1)
    p2 = fn.canvasCoords(canvas, elem.p2)
    Lcanvas = fn.distance(p1, p2)
    theta = elem.theta
    k = fn.angleSign(theta)
    tAngle = fn.textAngle(theta * 180 / np.pi)
    ...
    # ตรวจทิศทาง element
    reverse = p2[0] < p1[0] or p2[1] < p1[1]
    if reverse:
        p1, p2 = p2, p1

    # เลือก loadcase
    case = canvas.currentLoadcase if canvas.currentLoadcase != -1 else (
        len(canvas.loadcasesList) + canvas.currentCombination
    )

    # ดึง moment array
    M = canvas.forces[case][member][2]
    if reverse:
        M = [-m for m in M[::-1]]  # กลับทิศและค่าถ้าจำเป็น

    # แปลงหน่วย
    Mconverted = [fn.unitConvert('kN.cm', canvas.units[2], m) for m in M]

    # วาด polyline
    n = len(M)
    points = []
    for i in range(n):
        x = p1[0] + (i / (n - 1)) * (p2[0] - p1[0])
        y = p1[1] + f * M[i]
        pt = fn.rotate([x, y], p1, theta)
        points.append(pt)

    canvas.canvas.create_line(points, fill='green')

    # วาดตัวเลขที่ปลาย
    if abs(M[0]) > 0.1:
        textpos = fn.rotate([p1[0], p1[1] + f * M[0] - np.sign(M[0]) * 12], p1, theta)
        canvas.canvas.create_text(textpos, text='{:.2f}'.format(abs(Mconverted[0])),
                                  fill='green', angle=tAngle)

    if abs(M[-1]) > 0.1:
        textpos = fn.rotate([p2[0], p2[1] + f * M[-1] - np.sign(M[-1]) * 12], p2, theta)
        canvas.canvas.create_text(textpos, text='{:.2f}'.format(abs(Mconverted[-1])),
                                  fill='green', angle=tAngle)
